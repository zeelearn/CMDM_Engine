...
public partial class Lession_Plan_New : System.Web.UI.Page
{
    ...
    private void FillTopic_Add()
    {
        try
        {
            ...
            BindListBox(lstboxtopic_add, dsGrid, "Topic_Name", "Topic_Code");
    ...
    private void FillSubTopic_Add()
    {
        try
        {
            ...
            string TopicCode = "";
            for (int cnt = 0; cnt <= lstboxtopic_add.Items.Count - 1; cnt++)
            {
                if (lstboxtopic_add.Items[cnt].Selected == true)
                {
                    TopicCode = TopicCode + lstboxtopic_add.Items[cnt].Value + ",";
                }
            }

            if (TopicCode.Length > 1)
            {
                TopicCode = TopicCode.Substring(0, TopicCode.Length - 1);
            }
            else
            {
                TopicCode = "";
            ...
            BindListBox(lstboxsubtopic_add, dsGrid, "SubTopic_Name", "SubTopic_Code");
    ...
    private void FillModule_Add()
    {
        try
        {
            ...
            for (int cnt = 0; cnt <= lstboxsubtopic_add.Items.Count - 1; cnt++)
            {
                if (lstboxsubtopic_add.Items[cnt].Selected == true)
                {
                    SubTopicCode = SubTopicCode + lstboxsubtopic_add.Items[cnt].Value + ",";
                }
            }


            if (SubTopicCode.Length > 1)
            {
                SubTopicCode = SubTopicCode.Substring(0, SubTopicCode.Length - 1);
            }
            else
            {
                SubTopicCode = "";
            ...
            DataSet dsGrid = ProductController.Get_ModuleDetail_New("", DivisionCode, StandardCode, SubjectCode, ChapterCode, TopicCode, SubTopicCode, "3");
    ...
    private bool ValidationHeader()
    {
        ...
        if (lstboxtopic_add.SelectedIndex == 0 || lstboxtopic_add.SelectedIndex==-1)
        {
            Show_Error_Success_Box("E", "Select Topic");
            lstboxtopic_add.Focus();
            flag = false;
            return flag;
        }
        if (lstboxsubtopic_add.SelectedIndex == 0||lstboxsubtopic_add.SelectedIndex==-1)
        {
            Show_Error_Success_Box("E", "Select Sub Topic");
            lstboxsubtopic_add.Focus();
            flag = false;
            return flag;
        }
    ...
    private void SaveLessonHeader()
    {
        try
        {
            ...
            SubTopicCode = SubTopicCode.Substring(0, TopicCode.Length - 1);
        ...
        try
        {
          ...
          //  TopicCode = ddlTopic_Add.SelectedValue;
           ...
           // SubTopicCode = ddlSubTopic_Add.SelectedValue;
            ...
            string Result = string.Empty;
            Show_Error_Success_Box("E", "Kindly Coordinate with Administrator");
    ...
    private void ClearLessonHeader()
    {
        try
        {
            ...
            lstboxtopic_add.Items.Clear();
            lstboxsubtopic_add.Items.Clear();
            ddlChapter_Add.Items.Clear();
            ddlChapter_Add.Enabled = true;
            lstboxtopic_add.Enabled = true;
            lstboxsubtopic_add.Enabled = true;
    ...
    protected void dlGridDisplay_ItemCommand(object source, DataListCommandEventArgs e)
    {
        try
        {
            ...
            if (e.CommandArgument.ToString() != "")
            {
                ...
                DataSet ds = ProductController.GetLessonPlanHeader(Pkey, 3);
                ...
                if (ds != null)
                {
                    if (ds.Tables.Count != 0)
                    {
                        ...
                        lstboxtopic_add.Enabled = false;
                        lstboxsubtopic_add.Enabled = false;
                        ...
                        if (ds.Tables[1].Rows.Count > 0)
                        {
                            ...
                            {
                                for (int rcnt = 0; rcnt <= lstboxtopic_add.Items.Count - 1; rcnt++)
                                {
                                    if (lstboxtopic_add.Items[rcnt].Value == ds.Tables[1].Rows[cnt]["TopicCode"].ToString())
                                    {
                                        lstboxtopic_add.Items[rcnt].Selected = true;
                                        break;
                                    }
                                }
                            }
                        ...
                        if (ds.Tables[1].Rows.Count > 0)
                        {
                            for (int cnt = 0; cnt <= ds.Tables[1].Rows.Count - 1; cnt++)
                            {
                                for (int rcnt = 0; rcnt <= lstboxsubtopic_add.Items.Count - 1; rcnt++)
                                {
                                    if (lstboxsubtopic_add.Items[rcnt].Value == ds.Tables[1].Rows[cnt]["SubTopicCode"].ToString())
                                    {
                                        lstboxsubtopic_add.Items[rcnt].Selected = true;
                                        break;
                                    }
                                }
                            }
                        ...
                        lblPkey.Text = ds.Tables[0].Rows[0]["LessonPlanCode"].ToString();
    ...
    protected void BtnSaveAdd_Click(object sender, EventArgs e)
    {
        ...
        //if (TopicCode == "")
        //{
        //    Show_Error_Success_Box("E", "Atleast one Topic should be selected");
        //    lstboxtopic_add.Focus();
        //    return;
        //}
        ...
        //if (SubTopicCode == "")
        //{
        //    Show_Error_Success_Box("E", "Atleast one Sub Topic should be selected");
        //    lstboxsubtopic_add.Focus();
        //    return;
        //}
        if (TopicCode.Length > 1)
        ...
        if (lblPkey.Text == "")
        {
            Result = ProductController.Insert_UpdateLessonPlanHeader_New(DivisionCode, StandardCode, SubjectCode, ChapterCode, TopicCode, SubTopicCode, "", txtLessonPlanName_Add.Text.Trim(), txtDisplayName_Add.Text.Trim(), txtDescription_Add.Text.Trim(), EOD, IsActive, UserID, Pkey, "1", ModuleCode);
            ...
            else
            {
                ...
                Send_lessonPlan_Details_LMS(DivisionCode, StandardCode, SubjectCode, ChapterCode, txtLessonPlanName_Add.Text.Trim());
        ...
        {
            //string[] chrSpit = lblPkey.Text.Split('%');
            string LessonPlanCode = lblPkey.Text;
            Result = ProductController.Insert_UpdateLessonPlanHeader_New(DivisionCode, StandardCode, SubjectCode, ChapterCode, TopicCode, SubTopicCode, LessonPlanCode, txtLessonPlanName_Add.Text.Trim(), txtDisplayName_Add.Text.Trim(), txtDescription_Add.Text.Trim(), EOD, IsActive, UserID, Pkey, "2", ModuleCode);
    ...
    protected void lstboxtopic_add_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            lstboxsubtopic_add.Items.Clear();
    ...
    protected void lstboxsubtopic_add_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            ddlModule.Items.Clear();
    ...
    protected void ddlModuleAdd_SelectedIndexChanged(object sender, EventArgs e)
    {

    }


    private void Send_lessonPlan_Details_LMS(string DivisionCode,  string StandardCode, string SubjectCode, string ChapterName, string Lessonplan)
    {
        ...
        try
        {
            DataSet dsdetails = ProductController.GET_lessonPlan_DETAILS(DivisionCode, StandardCode, SubjectCode, ChapterName, Lessonplan);
            if (dsdetails.Tables[0].Rows.Count > 0)
            {
                for (int cnt = 0; cnt <= dsdetails.Tables[0].Rows.Count - 1; cnt++)
                {
                    ...
                    var LessonplanModuledetailsinsert = new LessonModulelist();
                    LessonplanModuledetailsinsert.LessonModuleId = null;
                    LessonplanModuledetailsinsert.ModuleCode = dsdetails.Tables[0].Rows[cnt]["ModuleCode"].ToString();
                    LessonplanModuledetailsinsert.LessonPlanCode = dsdetails.Tables[0].Rows[cnt]["LessonPlanCode"].ToString();
                    LessonplanModuledetailsinsert.CreatedOn = Convert.ToDateTime(dsdetails.Tables[0].Rows[cnt]["Created_On"]);
                    LessonplanModuledetailsinsert.ModifiedOn = Convert.ToDateTime(dsdetails.Tables[0].Rows[cnt]["Created_On"]);
                    LessonplanModuledetailsinsert.CreatedBy = UserID;
                    LessonplanModuledetailsinsert.ModifiedBy = UserID;
                    LessonplanModuledetailsinsert.IsActive = Convert.ToBoolean(dsdetails.Tables[0].Rows[cnt]["IsActive"]);
                    LessonplanModuledetailsinsert.IsDeleted = false;

                    var response = client.PostAsJsonAsync("LessonModule/addUpdLessonModule", LessonplanModuledetailsinsert).Result;
    ...
    class LessonModulelist
    {
        public string LessonModuleId { get; set; }
