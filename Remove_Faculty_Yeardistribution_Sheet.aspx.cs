...
public partial class Remove_Faculty_Yeardistribution_Sheet : System.Web.UI.Page
{
    ...
    private void FillDDL_Centre()
    {
        ...
        BindDDL(ddlCenter_Add, dsCentre, "Center_Name", "Center_Code");
                ...
                foreach (DataListItem dtlItem in dlGridChapter.Items)
                {
                    ...
                    if (chkitemck.Checked == true)
                    {
                        ...
                        Label lblDLChapterCode = (Label)dtlItem.FindControl("lblDLChapterCode");
                         Label lblParnerCode = (Label)dtlItem.FindControl("lblParnerCode");
                         Label lblbatchcode = (Label)dtlItem.FindControl("lblbatchcode");
                                         
                        Label lblPkcode = (Label)dtlItem.FindControl("lblPkcode");
                        Label lblResult = (Label)dtlItem.FindControl("lblResult");
                        lblResult.Text = "";
                        int ResultId = 0;
                        //if (Teacher_Code.Trim() != "")
                        //{
                        ResultId = ProductController.Delete_YearDistribution(DivisionCode, Acad_Year, Standard_Code, LMSProductCode, SchedulHorizonTypeCode, CenterCode, SubjectCode, lblDLChapterCode.Text, lblParnerCode.Text, CreatedBy, lblbatchcode.Text, lblPkcode.Text);
    ...
    //    lblDivision_Add.Text = "";
    //    lblAcadYear_Add.Text = "";
    //    lblCourse_Add.Text = "";
    //    lblLMSProduct_Add.Text = "";
    //    lblSubject_Add.Text = "";
    //    lblSchedulingHorizon_Add.Text = "";
    //    dlGridChapter.DataSource = null;
    //    dlGridChapter.DataBind();
    ...
    private void Fill_Assign_Teacher()
    {
        ...
        if (ddlCenter_Add.SelectedItem.Value != "Select")
        {
            ...
            DataSet dsGrid = ProductController.GetYearDistributionsheetBy_Division_Year_Standard_Subject_CenterFORREMOVE(DivisionCode, AcedYear, ddlSubjectName.SelectedItem.Value, ddlCenter_Add.SelectedItem.Value, StandardCode, ddlSchHorizon.SelectedItem.Value, LmsProductCode, BatchCode);
            ...
            if (dsGrid != null)
            {
                if (dsGrid.Tables.Count != 0)
                {
                    ...
                    lblAcadYear_Add.Text = ddlAcademicYear.SelectedItem.ToString();
                    lblCourse_Add.Text = ddlCourse.SelectedItem.ToString();
                    lblLMSProduct_Add.Text = ddlLMSProduct.SelectedItem.ToString();
                    lblSubject_Add.Text = ddlSubjectName.SelectedItem.ToString();
                    lblSchedulingHorizon_Add.Text = ddlSchHorizon.SelectedItem.ToString();
                    lblbatchname.Text = ddlBatch.SelectedItem.ToString();
                    //int count = 0;
